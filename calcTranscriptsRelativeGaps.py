######################################
# Calculate Transcripts Using Genomecov
# Author: Mika Olami
...
current_start=0  # transcrript start
current_end=0  # transcipt end
current_chr=''  # current chromosome
cov_peak=0  # current coverage peak
bed_dict = {}  # dictionary that maps chromosomes to a list of [start, end] transcripts
gap_count = 0  # gap counter

# how to use
if len(sys.argv) < 4:
    exit("Usage: python3 calcTranscriptsRelativeGaps.py [genomecov results] [output file] "
         "[min coverage # per tanscript] [max gap length]")

min_coverage = int(sys.argv[3])  # minimum coverage allowed for transcripts detection
...
with open(sys.argv[1],"r") as genomecov_coverge:
    ...
    for base in genomecov_coverge:
        ...
        if float(base[2]) >= min_coverage:  # check the coverage
            cov_peak=max(float(base[2]), cov_peak)  # check for new coverage peak
            min_coverage=max(math.ceil(cov_peak*0.05), min_coverage)  # get the coverage counted as gaps
            if len(current_chr) == 0 or current_chr != base[0]:  # check if new chromosome or new transcript
                if len(current_chr)>0 and [current_start,current_end] not in bed_dict[current_chr]:  # check if transcript not appended
                    bed_dict[current_chr].append([current_start, current_end])  # append transcript to chromosome
                current_chr = base[0]  # current chromosome
                cov_peak=float(base[2])  # reset coverage peak for next transcript
            ...
            if gap_count > max_gap:
                if len(current_chr) > 0:
                    ...
                    gap_count = 0  # reset gap counter
                    cov_peak = 0  # reset coverage peak
                    min_coverage = int(sys.argv[3])  # reset coverage counted as gaps
